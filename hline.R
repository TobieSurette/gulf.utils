...
hline <- function(x, lower, upper, ...){
   ...
   if (missing(lower)) lower <- par("usr")[1]
   if (missing(upper)) upper <- par("usr")[2]
   
   # Remove lines which are not in the plot region:
   x <- x[(x >= par("usr")[3]) & (x <= par("usr")[4])]
   ...
   if (length(x) > 0){
      ...
      for (i in 1:length(x)) lines(c(lower[i], upper[i]), rep(x[i], 2), ...) 
   }
} 

vline <- function(x, lower, upper, ...){
   # Define default line bounds:
   if (missing(lower)) lower <- par("usr")[3]
   if (missing(upper)) upper <- par("usr")[4]
   
   # Remove lines which are not in the plot region:
   x <- x[(x >= par("usr")[1]) & (x <= par("usr")[2])]
   ...
   if (length(x) > 0){
      ...
      if (any(lower > upper)) stop("Lower bounds cannot be greater than upper bounds.")
      if (length(lower) != length(x)) stop("Number of lines and bounds are inconsistent.")
      for (i in 1:length(x)) lines(rep(x[i], 2), c(lower[i], upper[i]), ...) 
   }
} 
